...
public class PageLoader<ListItem, DataEntity> implements ICataloguePage2<ListItem, DataEntity>,
     ...
     * 翻页或者刷新时触发的回调
     */
    private FlipListener myFlipListener = null;
     ...
     */
    @NonNull
    private final List<ListItem> data = createFinalBaseData();

    private final IRead realRead = new IRead() {
        @Override
        public boolean isEnd() {
            return PageLoader.this.isEnd();
        }

        @Override
        public int getBusinessMode() {
            return PageLoader.this.getBusinessMode();
        }
...
//        isLoaderNow = false;
     ...
     * @return
     */
    public static <I, D> ICataloguePage2<I, D> createInstance(@NonNull PageCallBack<I, D> callBack, @NonNull PageFactory pageFacroty) {
        return pageFacroty.createInstance(callBack);
    }

    public static <I, D> ICataloguePage2<I, D> createInstance(@NonNull PageCallBack<I, D> callBack) {
        return new PageFactory() {
        }.createInstance(callBack);
    }

    static <T, D> ICataloguePage2<T, D> instance(PageCallBack<T, D> callBack) {
        return new PageLoader<>(callBack);
    }


    protected PageLoader(PageCallBack<ListItem, DataEntity> pageCallBack) {
        setPageCallBack(pageCallBack, true);
    }
    ...
    public void setPageCallBack(@NonNull PageCallBack<ListItem, DataEntity> pageCallBack) {
        setPageCallBack(pageCallBack, true);
    }

    public void setPageCallBack(@NonNull PageCallBack<ListItem, DataEntity> pageCallBack, boolean isBindCallBackLifecycleOwner) {
        this.pageCallBack = pageCallBack;
        if (isBindCallBackLifecycleOwner) {
            updateLifecycle(this.pageCallBack.getThisLifecycle());
        }
    }

    private void updateLifecycle(LifecycleOwner lifecycleOwner) {
        //设置刷新/加载更多回调
    ...
    private void load(int page) {
        ...
        int p = page;
        /**
         * 如果当前的判断-数据是否空的函数@{@link checkEndDatasFunction}为空的话。
         * 这里的{@link PageCallBack#convertPage(int, int, int, int)} 就已经失效。
         * 因为这里判断是根据pagesize来取舍的，当用这个来判断数据end时候{@link ICheckDataEnd#checkDataIsNull(List)} pagesize已经失去意义
         * 这里取舍有一定的问题，但是暂时这样设置
         */
    ...
    @Override
    public void next() {
        FlipListener flipListener = getFlipListener();
        if (flipListener != null) {
            if (flipListener.onLoadMoreBefore(realRead)) {
                return;
            }
        }
        nextNewMethod(true);
        ...
        if (isEnableLimiteMoreLoadByMethodPara) {
            if (enableNextLoadLimit) {
                if (Math.abs(thisNextTime - System.currentTimeMillis()) <= thisNextTimeLimite) {
                    Log.d(TAG, "refresh: 加载下一页间隔时间短~ , thisNextTime = " + thisNextTime + " , thisNextTimeLimite = " + thisNextTimeLimite);
                    return;
                }
            }
        }
    ...
    public void refresh() {
        FlipListener flipListener = getFlipListener();
        if (flipListener != null) {
            if (flipListener.onRefreshBefore(realRead)) {
                return;
            }
        }
        refreshNewMethod(true);
...
//                return checkEndDatasFunction != null && checkEndDatasFunction.checkDataIsNull(null);
    ...
    private Set<RemoteObserver<DataEntity>> getRemoteObservers() {
        return mRemoteObservers;
    }
    ...
    @NonNull
    protected List<ListItem> createFinalBaseData() {
        return new PageArrayList<>();
    }
